defmodule JellystoneWeb.TagLive.FormComponent do
  ...
  def update(%{tag: tag} = assigns, socket) do
    changeset = Databases.change_tag(tag)
  ...
  def handle_event("validate", %{"tag" => tag_params}, socket) do
    changeset =
      socket.assigns.tag
      |> Databases.change_tag(tag_params)
  ...
  defp save_tag(socket, :edit, tag_params) do
    case Databases.update_tag(socket.assigns.tag, tag_params) do
      {:ok, tag} ->
  ...
  defp save_tag(socket, :new, tag_params) do
    case Databases.create_tag(tag_params) do
      {:ok, tag} ->
        notify_parent({:saved, tag})
