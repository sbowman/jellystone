defmodule JellystoneWeb.SiteLive.FormComponent do
  ...
  def update(%{site: site} = assigns, socket) do
    changeset = Databases.change_site(site)
  ...
  def handle_event("validate", %{"site" => site_params}, socket) do
    changeset =
      socket.assigns.site
      |> Databases.change_site(site_params)
  ...
  defp save_site(socket, :edit, site_params) do
    case Databases.update_site(socket.assigns.site, site_params) do
      {:ok, site} ->
  ...
  defp save_site(socket, :new, site_params) do
    case Databases.create_site(site_params) do
      {:ok, site} ->
        notify_parent({:saved, site})
